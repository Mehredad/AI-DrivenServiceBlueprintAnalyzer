...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE UNIQUE INDEX ix_waitlist_email_lower ON waitlist_signups (lower(email))"
    )
    op.create_index("ix_waitlist_created", "waitlist_signups", ["created_at"])
    op.create_index("ix_waitlist_status", "waitlist_signups", ["status"])
